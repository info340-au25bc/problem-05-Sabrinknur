...
function huskiesLost(game) {
  // if UW is home, they lost if opponent_score > home_score, else inverse
  if (game.home === 'UW') return game.opponent_score > game.home_score;
...
console.log(JSON.stringify(lostGames.map(extractOpponent)));
...
lostGames.forEach(function(g){
  if (g.home === 'UW') {
    console.log(`${g.opponent} at UW, ${g.opponent_score} to ${g.home_score}`);
  } else {
    console.log(`${g.home} vs UW, ${g.home_score} to ${g.opponent_score}`);
  }
});
...
function mostYardsPassing(g1, g2) {
  if (typeof g1.passing_yards !== 'number') return g2;
  return (g2.passing_yards > g1.passing_yards) ? g2 : g1;
}
